...
# 更新脚本
nowversion=4.1
wget -q --no-check-certificate 'https://raw.githubusercontent.com/zjw2017/odex-For-MIUI-WeeklyReleases/master/version'
latestversion=$(cat version)
latestshname="odex%20V$(cat version).sh"
lastname="odex V$(cat version).sh"
latesturl="https://raw.githubusercontent.com/zjw2017/odex-For-MIUI-WeeklyReleases/master/$latestshname"
if [ $nowversion != $latestversion ] ; then
   wget -q --no-check-certificate $latesturl
   if [ $? -eq 0 ]; then
      echo "- 新版本已下载完毕，请退出运行odex V$latestversion.sh"
      rm -rf version
	  mv "$latestshname" "$lastname"
      exit
   fi
else
   echo "已是最新版本"
